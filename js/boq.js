(function initializeBoqEditor() {
  ...
  function desktopRow(item, displayIndex) {
    ...
    return `<tr data-item-row data-item-id="${item.id}">
      ...
      <td class="calculated-cell column-cogs column-price" data-item-output="totalCogs">${formatBoqAmount(calc.totalCogs, currentCurrency())}</td>
      ...
      <td class="calculated-cell column-selling column-price" data-item-output="totalSelling">${formatBoqAmount(calc.totalSelling, currentCurrency())}</td>
  ...
  function categorySubtotalRow(category) {
    ...
    return `<tr class="category-subtotal-row" data-category-subtotal="${escapeHtml(category)}"><td colspan="7">${escapeHtml(category)} subtotal</td><td class="align-right column-cogs column-price" data-category-total="cogs">${formatBoqAmount(summary.totalCogs, currentCurrency())}</td><td class="column-margin column-price"></td><td class="column-selling column-price"></td><td class="align-right column-selling column-price" data-category-total="selling">${formatBoqAmount(summary.totalSelling, currentCurrency())}</td><td></td></tr>`;
  }
  ...
  function mobileCard(item, displayIndex) {
    ...
    return `<article class="mobile-item-card" data-item-row data-item-id="${item.id}">
      ...
      <div class="mobile-item-body">
        ...
        <div class="mobile-item-total column-selling column-price"><span class="muted">Total selling</span><strong data-item-output="totalSelling">${formatBoqAmount(calc.totalSelling, currentCurrency())}</strong></div>
      </div>
    </article>`;
  }
    ...
    document.querySelectorAll(`[data-item-row][data-item-id="${CSS.escape(item.id)}"]`)
      .forEach((row) => {
        row.querySelectorAll('[data-item-output="totalCogs"]').forEach((cell) =>
          cell.textContent = formatBoqAmount(calc.totalCogs, currentCurrency())
        );
        row.querySelectorAll('[data-item-output="totalSelling"]').forEach((cell) =>
          cell.textContent = formatBoqAmount(calc.totalSelling, currentCurrency())
        );
  ...
  function updateCategorySubtotal(category) {
    ...
    document.querySelectorAll(`[data-category-subtotal="${CSS.escape(category)}"]`)
      .forEach((row) => {
        ...
        if (cogs) cogs.textContent = formatBoqAmount(summary.totalCogs, currentCurrency());
        if (selling) selling.textContent = formatBoqAmount(summary.totalSelling, currentCurrency());
      });
  }
